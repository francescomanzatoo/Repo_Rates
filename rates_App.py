...
def valueComparision():
    return pd.read_sql_query("""
        ...
        FROM positions p
        JOIN rates AS r_ins
            ON r_ins.currency = p.currency
            AND r_ins.data = (SELECT MAX(data) FROM rates r2
                              WHERE r2.currency = p.currency
                                AND r2.data <= p.data)
        JOIN rates AS r_now
            ON r_now.currency = p.currency
            AND r_now.data = (SELECT MAX(data) FROM rates r3
                              WHERE r3.currency = p.currency)
        """, connection)
